  ...
  function test() {
  	...
  	if (!revealed) {
  		...
  		if (serverStatus === true) httpGet("https://example.com", function() {
  			return;
  		});
  	} else {
  		setTimeout(function() {
  ...
  function newVok() {
  	...
  	httpGet("https://lernen.nicolaiweitkemper.de/Vokabeln/Backend/get.php?level=" + level + "&course=" + course + "&last=" + current + "&username=" + username + '&request={"type":"prepositions"}',
  		function(result) {
  			...
  			current = vok["preposition"];
  			currentQ = vok["before"] + "?" + vok["after"];
  			document.getElementById("abfrage").innerHTML = currentQ;
  			document.getElementById("eingabe").value = "";
  			document.getElementById("hint").value = vok["ger"];
  ...
  function reveal(bad) {
		...
		if (bad) httpGet("https://example.com", function() {});
  ...
  function onLoad() {
  	...
  	if (URLcourse === null) window.location.href = "lektion-auswahl.html";
  	...
  	httpGet("https://lernen.nicolaiweitkemper.de/Vokabeln/Backend/userdata-get.php?username=" + username, function(result) {
  ...
  function ttsPrepare() {
  	...
  	if (!ttsAudioCache[text]) {
  		...
  		ttsAudio = new Audio(
  			...
  			"https://google-translate-proxy.herokuapp.com/api/tts?query=" + encodeURIComponent(text) + "&language=es" :
  			//TODO &speed=0.24
  ...
  function editHintFinish() {
  	...
  	httpGet("https://lernen.nicolaiweitkemper.de/Vokabeln/Backend/vok-hint-change.php?spa=" + encodeURIComponent(current) + "&hint=" + encodeURIComponent(hint) + '&course=' + course + "&username=" + username + "&level=" + level, function() {});
  }
